...
const Amazing = () => {
  ...
  // API Calls
  // ============================================
  const { data: productsData, isLoading: productsLoading, error: productsError } = useGetProductsOnSaleQuery({ 
    perPage: 100 
  });
  ...
  const bannerData = useMemo(() => {
    ...
    return banners.find(b => Number(b.collection_id) === 8) || banners[0] || null;
  }, [popularBannersData]);
  ...
  return (
    <div className="min-h-screen" dir="rtl">
      ...
      <ProductList
        ...
        error={productsError}
        banner={bannerData}
        bannerTitle="تخفیف‌های ویژه"
        bannerIcon={<Sparkles className="w-6 h-6 text-yellow-300" />}
        bannerColor="from-pink-500 to-red-500"
        title="تخفیف‌های ویژه و شگفت‌انگیز"
        emptyMessage="محصول شگفت‌انگیزی یافت نشد"
        emptyIcon="🎯"
        colorScheme="pink"
        itemsPerPage={20}
        showRank={false}
